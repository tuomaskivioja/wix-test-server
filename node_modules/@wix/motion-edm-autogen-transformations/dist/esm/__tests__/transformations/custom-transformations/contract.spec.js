...
describe('transformation contract tests', () => {
  ...
  `('$transformationFunction.name - omitting fields', ({ transformationFunction, expression, sourceFactory, undefinedFallback, }) => {
        ...
        `('handles a top-level object to omit that $description', ({ invalidSource, expected }) => {
        ...
        `('does nothing when the nested-path object to omit from $description', ({ rawSource, expected }) => {
    ...
    `('$transformationFunction.name - supports complex expressions', ({ transformationFunction, sourceFactory, expression }) => {
            const [propertyToOmit, valueToOmit, propertyToKeep, valueToKeep] = times(4, () => aString());
    ...
    `(
        ...
        '$transformationFunction.name - flattens argument object keys onto its parent object', ({ transformationFunction, transformation, source, expectedFactory }) => {
    ...
    `(
        /* eslint-enable max-len */
        '$transformationFunction.name - flattens top-level argument object-keys onto their parent object', ({ transformationFunction, transformation, source, expectedFactory }) => {
            expect(transformationFunction(source, transformation)).toStrictEqual(expectedFactory(source));
        });
        /* eslint-disable max-len */
        test.each `
    ...
    `(
        /* eslint-enable max-len */
        '$transformationFunction.name - flattens nested argument object-keys onto their parent object', ({ transformationFunction, transformation, source, expectedFactory }) => {
            expect(transformationFunction(source, transformation)).toStrictEqual(expectedFactory(source));
        });
      ...
      `('$transformationFunction.name - does not override sibling properties at the spread target', ({ transformationFunction, transformation, source }) => {
                /* eslint-enable max-len */
                expect(transformationFunction(source, transformation)).toStrictEqual({
                    someValue,
                    b: {
      ...
      `('$transformationFunction.name - does not override nested sibling properties at the spread target', ({ transformationFunction, transformation, source }) => {
                /* eslint-enable max-len */
                expect(transformationFunction(source, transformation)).toStrictEqual({
                    topLevel: {
                        someValue,
      ...
      `(
            /* eslint-enable max-len */
            '$transformationFunction.name - passes the input as is when the value to spread does not exist', ({ transformationFunction, transformation, source, expectedFactory, }) => {
                expect(transformationFunction(source, transformation)).toStrictEqual(expectedFactory(source));
            });
            /* eslint-disable max-len */
            test.each `
      ...
      `(
            /* eslint-enable max-len */
            '$transformationFunction.name - does nothing when the value to spread is not an object', ({ transformationFunction, transformation, source, expectedFactory, }) => {
                expect(transformationFunction(source, transformation)).toStrictEqual(expectedFactory(source));
            });
        });
    });
  ...
  `('$transformationFunction.name - custom functions', ({ transformationFunction, idJsonPath, sourceFactory }) => {
        const id = aString();
    ...
    describe('array items transformation', () => {
        ...
        const sourceBase = set(validSource, `${sourceObjectPath}.${nonArrayPath}`, nonArrayPathValue);
        describe.each `
    ...
    `('$transformationFunction.name', ({ transformationFunction, rootPathExpression, source, expectedForNonExistingPath, }) => {
            ...
            test('supports custom functions applied on items - top-level', () => {
                ...
                const transformationResult = transformationFunction(source, transformation, { addOne: (x) => x + 1 });
          ...
          `('does not fail when a top-level path $description', ({ path, expected }) => {
                    const transformation = {
            ...
            ${nonArrayPath}   | ${nonArrayPathValue} | ${'is not an array'}
          `('does not fail when a nested path $description', ({ path, expected }) => {
                    const transformation = set({}, transformationPath, {
                        '@path': `${rootPathExpression}.${path}`,
                        '@itemTransformation': '$.aaa',
                    });
    ...
    describe('constant transformation', () => {
      ...
      ${transformToRequestMessage} | ${`$[0].${sourceObjectPath}`} | ${(sourceValue) => [sourceValue]}
      ${transformResponseMessage}  | ${`$.${sourceObjectPath}`}    | ${(sourceValue) => sourceValue}
    `('$transformationFunction.name - omitting fields', ({ transformationFunction, expression, sourceFactory }) => {
