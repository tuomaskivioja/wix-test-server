...
const isApplicationError = ambassadorHTTPError => {
  const isStatusOfClientError = isClientError(ambassadorHTTPError == null ? void 0 : ambassadorHTTPError.httpStatus);
  return has(ambassadorHTTPError, 'response.details.applicationError') || isStatusOfClientError;
};
...
const buildValidationError = (ambassadorHTTPError, transformation, argumentNames) => {
  ...
  const message = `INVALID_ARGUMENT: ${transformedFieldViolations.map(({
    field,
    description
  }) => field ? `"${field}" ${description}` : `${description}`).join(', ')}`;
  const details = {
    validationError: {
