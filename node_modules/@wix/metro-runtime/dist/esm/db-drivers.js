...
export function toAmbassadorRequest({ rootSchema, depSchemas, fqdnTransformation }, entity) {
    const asRequestStructure = fromCorvidEntity(transformFqdn(entity, fqdnTransformation));
    const request = schemaSerializer(rootSchema, depSchemas, converters)(asRequestStructure, ConverterType.TO_JSON);
    return request;
}
export function toDbDriverEntity({ rootSchema, depSchemas, fqdnTransformation }, entity) {
    const asDbDriverStructure = schemaSerializer(rootSchema, depSchemas, converters)(entity, ConverterType.FROM_JSON);
    const dbDriverEntity = toCorvidEntity(transformFqdn(asDbDriverStructure, fqdnTransformation));
