...
export const videoV2 = {
    ...
    [ConverterType.FROM_JSON]: {
        transform: (val) => {
            ...
            if (val.posters?.length) {
                ...
                if (posterId) {
                    posterData = `${URL_HASH_PREFIX}posterUri=${posterId}&posterWidth=${poster.width}&posterHeight=${poster.height}`;
                }
            }
            return val.id
    ...
    [ConverterType.TO_JSON]: {
        transform: (val) => {
            ...
            const { posterHeight: height, posterWidth: width, posterUri, } = querystring.parse(hash.replace(URL_HASH_PREFIX, ''));
